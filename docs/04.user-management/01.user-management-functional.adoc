...
Conversly, the user's profile at a given EP also provides the ability to manage any inward authorizations they have granted to other EPs, i.e. ability to revoke a previous authorization by invalidating the refresh token.
...
#*[COMMENT(RAC): There is a design choice to be made here. In the absence of 'Policy Context Propagation', an alternative approach is for the Resource Server to directly interact with the Policy API to query information from the policy database in order to prepare an appropriately filtered response. In this case there is no need for the PEP to propagate policy information to the resource server. Further discussion required.]*#

===== NOTES from recent discussions

#*[COMMENT(RAC): This section includes some notes from recent discussions on this topic...]*#

There are two cases for resource requests hitting the PEP (protecting the resource server (e.g. ADES)):

* Case 1: Simple - The resource can be fully protected by the PEP based on the URL
* Case 2: Complex - The resource server behaviour and/or response relies upon policy information, (e.g. ADES GetProcesses repsonse filtered according to requesting user's access rights)

For Case 2) the question is how the resource server (e.g. ADES) obtains the required 'additional information' needed to fulfil the request. Options include...

* Option A: The PEP provides the 'additional information' by augmenting the request passed to the resource server, e.g. additional http header params
* Option B: The resource server initiates external queries to obtain the additional policy information required
* Option C: The PEP modifies the response from the resource server to ensure it is in accordance with the defined policy

Options A) & C) have the problem that the PEP requires to have knowledge regarding the function and behaviour of the resource server - i.e. to know which 'additional information' to provide, or to know when/how to modify the response. A PEP plugin approach could facilitate this.

Option C) has the additional limitation that it is only able to impact the response, and would not trigger any 'policy-based-behaviour' that might be important within the resource server.

*Option B) seems like the most robust choice as it relies upon a more generic PEP. The downside being the need for the resource server to implement the interactions required to query the policy database.* This query could be made directly to the policy database (PDP?) directly, or may also consider the PEP providing a simplified policy query endpoint.
